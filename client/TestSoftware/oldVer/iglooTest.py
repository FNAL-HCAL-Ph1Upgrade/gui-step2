...
def ones(): # "all ones register"
    wb.write(0x09,[0x02])
    wb.read(0x09,4)
    return strToHex(wb.sendBatch()[1])

# Register byte 0x03 (RO)
def zeros(): # "all zeros register"
    wb.write(0x09,[0x03])
    wb.read(0x09,4)
    return wb.sendBatch()[1]
...
def wte_count(): # "WTE Counter"
    wb.write(0x09, [0x14])
    wb.read(0x09,4)
    wtectr = wb.sendBatch()[1]
    return strToHex(wtectr)

# Register bytes 0x15-17 (RO)
def capIDErr_count(): # "Cap ID error Counter (Links 1-3)"
    wb.write(0x09, [0x15]) # (link 1)
    wb.read(0x09,4)
    link1 = wb.sendBatch()[1]

    wb.write(0x09, [0x16]) # (link 2)
    wb.read(0x09,4)
    link2 = wb.sendBatch()[1]

    wb.write(0x09, [0x17]) # (link 3)
    wb.read(0x09,4)
    link3 = wb.sendBatch()[1]
...
def fifo_data(): # "FIFO Data 1-3 (Reserved)"
    wb.write(0x09, [0x30]) # (data1)
    wb.read(0x09,11) # 88 bits = 11 bytes
    data1 = wb.sendBatch()[1]

    wb.write(0x09, [0x31]) # (data2)
    wb.read(0x09,11)
    data2 = wb.sendBatch()[1]

    wb.write(0x09, [0x32]) # (data3)
    wb.read(0x09,11)
    data3 = wb.sendBatch()[1]
...
def inputSpy(desiredReg = "all"): # default = display all of register
    wb.write(0x09, [0x33])
    wb.read(0x09,25) # 200 bits = 25 bytes
    reg = wb.sendBatch()[1]
...
def spy96bits(): # "Fw-dependent monitoring (from v3.04 it is the orbit-histo)"
    wb.write(0x09, [0x40])
    wb.read(0x09,12) # 96 wbits = 12 bytes
    spy = wb.sendBatch()[1]
    return strToHex(spy)

# Register bytes 0x60-6B (RW)
def qie_ck_ph(): # QIE1-12 Clock Phase (Valid values of 0-15)
    wb.write(0x09, [0x60]) # QIE1 clock phase
    wb.read(0x09,1)
    qie1 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x61]) # QIE2
    wb.read(0x09,1)
    qie2 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x62]) # QIE3
    wb.read(0x09,1)
    qie3 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x63]) # QIE4
    wb.read(0x09,1)
    qie4 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x64]) # QIE5
    wb.read(0x09,1)
    qie5 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x65]) # QIE6
    wb.read(0x09,1)
    qie6 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x66]) # QIE7
    wb.read(0x09,1)
    qie7 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x67]) # QIE8
    wb.read(0x09,1)
    qie8 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x68]) # QIE9
    wb.read(0x09,1)
    qie9 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x69]) # QIE10
    wb.read(0x09,1)
    qie10 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x6A]) # QIE11
    wb.read(0x09,1)
    qie11 = strToHex(wb.sendBatch()[1])

    wb.write(0x09, [0x6B]) # QIE12
    wb.read(0x09,1)
    qie12 = strToHex(wb.sendBatch()[1])
...
def addrToSERDES(): # "bits[13:2] tied to SERDES APB_S_PADDR[13:2]"
    wb.write(0x09,[0x83])
    wb.read(0x09,2)
    return strToBin(wb.sendBatch()[1])
